...
def main():
    ...
    if dx == 25:
        ...
        path_tracer_file = os.path.join(path_data, 'tracer_k' + str(k0) + '_interpol/output/', 'coldpool_tracer_out.txt')
    ...
    #                 times, path_out_figs)
    timerange = [600, 1200, 1800, 2100, 2400, 3000]
...
def plot_vrad_w(r_av, U_rad_av, radius_rad_av,
    ...
    #     ax0.plot([r_av[it, k0_tracer], r_av[it, k0_tracer]], [-10, 10], ':', color='0.25',
    ...
    #         ax0.plot(radius_rad_av[ir_tracer_0], v_rad_av[it, ir_tracer_0], 'go', label='tracer: t')  # tracer
    #         ax0.plot(radius_rad_av[ir_tracer_1], v_rad_av[it, ir_tracer_1], 'ko', label='tracer: t+1')  # tracer
    #         # ax0.plot(radius_rad_av[ir_tracer_2], v_rad_av[it, ir_tracer_2], 'bo', label='tracer: t+2')  # tracer
    #
    #     else:
    ...
    #     ax1.plot([r_av[it, k0_tracer], r_av[it, k0_tracer]], [-10, 10], ':', color='0.25',
    #              linewidth=1)  # line through tracer
    ...
    #     # ax1.plot(radius_rad_av[ir_tracer_2], w_av[it, ir_tracer_2], 'bo', markersize=5)  # tracer
    #
    # ax0.legend(loc='center left', bbox_to_anchor=(.76, 0.87), frameon=False)
    # ax1.legend(loc='center left', bbox_to_anchor=(.76, 1.63), frameon=False)
    # rect = mpatches.Rectangle((6.7e3, 2.1), 2.e3, 6.2, fill=True, linewidth=1, edgecolor='k', facecolor='white')
    # ax0.add_patch(rect)
    # ax0.set_ylim(np.amin(v_rad_av), np.amax(v_rad_av))
    # ax1.set_ylim(-1.2, 1.5)
    # for ax in axis.flat:
    ...
    rect = mpatches.Rectangle((6.8e3, 2.4), 1.9e3, 5.9, fill=True, linewidth=1, edgecolor='k', facecolor='white')
    ax0.add_patch(rect)

    ax0.text(4.9e2, 7.2, 'a)', fontsize=18)
