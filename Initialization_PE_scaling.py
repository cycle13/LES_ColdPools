...
def main():
    ...
    ax1.plot(dTh_range, PE_ref * np.ones(n_thermo), 'k-', linewidth=3)
    ax2.plot(dTh_range, np.ones(n_thermo), 'k-', linewidth=3)
    ax1_.plot(rstar_range, PE_ref * np.ones(n_geom_r), 'k-', linewidth=3)
    ax2_.plot(rstar_range, PE_ref * np.ones(n_geom_r), 'k-', linewidth=3)

    PE = np.zeros((n_thermo, n_geom_r))
    ...
    for i, s in enumerate(scaling):
        ...
        PE_range[i] = PE_ref * s
        ax1.plot(dTh_range, PE_range[i] * np.ones(n_thermo), 'k-', linewidth=1.5)
        ax2.plot(dTh_range, PE_range[i] * np.ones(n_thermo) / PE_ref, 'k-', linewidth=1.5)
        ax1.fill_between([dTh_range[0], dTh_range[-1]],
        ...
        ax2.fill_between([dTh_range[0], dTh_range[-1]],
                         scaling[i]-0.1, scaling[i]+0.1,
                         color='gray', alpha=0.4, label=r'$\pm$ 0.1 PE_ref')
        ax3.fill_between([dTh_range[0], dTh_range[-1]],
                         scaling[i] - 0.1, scaling[i] + 0.1,
                         color='gray', alpha=0.4, label=r'$\pm$ 0.1 PE_ref')
    ...
    for ir, rstar in enumerate(rstar_range):
        ...
        for idTh, dTh in enumerate(dTh_range):
            ...
            for i, s in enumerate(scaling):
                ...
                if res < 0.05:
                    count[i] += 1
                    params_dict[str(s)] = np.append(params_dict[str(s)], [PE[idTh, ir], dTh, rstar, res]).reshape(count[i], 4)
            if PE[idTh, ir] > np.amax(scaling)*1.1*PE_ref:
                PE[idTh+1:, ir] = 20*PE_ref
                break
        ax1.plot(dTh_range, PE[:, ir], '-o', color=cm(icol))
        ax2.plot(dTh_range, PE[:, ir] / PE_ref, '-o', color=cm(icol),
                 label='r*=' + str(np.int(rstar)))
        ax3.plot(dTh_range, PE[:, ir] / PE_ref, '-', color='gray')
    for idTh, dTh in enumerate(dTh_range):
    ...
    for i, s in enumerate(scaling):
        for j in range(1,np.int(count[i])):
            ...
            ax3.plot(dth, pe/PE_ref, 'o', color=cm(res * 10),
                 label='s=' + str(np.round(s, 1)) + ', r*=' + str(np.int(rs))
                       + ', dTh=' + str(np.int(dth)) + ', res=' + str(np.round(res, 2)))
            ax3_.plot(rs, pe/PE_ref, 'o', color=cm(res * 10),
                  label='s=' + str(np.round(s, 1)) + ', r*=' + str(np.int(rs))
                        + ', dTh=' + str(np.int(dth)) + ', res=' + str(np.round(res, 2)))

    fsize = 15
    ax1.set_xlabel('dTh   [K]', fontsize=fsize)
    ax2.set_xlabel('dTh   [K]', fontsize=fsize)
    ax3.set_xlabel('dTh   [K]', fontsize=fsize)
    ax1.set_ylabel('PE(dTh)   [J]', fontsize=fsize)
    ax2.set_ylabel('PE(dTh)/PE_ref', fontsize=fsize)
    ax3.set_ylabel('PE(dTh)/PE_ref', fontsize=fsize)
    ax1.set_ylim(0, np.amax(scaling)*1.15*PE_ref)
    ax2.set_ylim(0, np.amax(scaling)+1)
    ax3.set_ylim(0, np.amax(scaling)+1)
    ax1.set_xlim(0, np.amax(dTh_range))
    ax2.set_xlim(0, np.amax(dTh_range))
    ax3.set_xlim(0, np.amax(dTh_range))
    ax1.set_xticks(dTh_range, minor=True)
    ax2.set_xticks(dTh_range, minor=True)
