...
def main():
    ...
    for it, t0 in enumerate(timerange):
        ...
        # fields
        plot_vorticity_field(vort_xz_, vort_yz_, icshift, jcshift, t0, kmax)
...
def plot_vorticity_field(vort_xz_, vort_yz_, icshift, jcshift, t0, kmax):
...
def plot_comparison_vort_vort_stag(vort_yz, vort_yz_, vort_yz_stag, jcshift, jc, kmax, t0):
    ...
    for i in range(3):
        ...
        ax.plot(np.arange(0, ny_), vort_yz_[:ny_, i], label='vort_yz_')
    ...
    for i in range(3):
        ax = axes[2, i]
        ax.plot(np.arange(0, ny_), vort_yz[jc - np.int(ny_ / 2):jc + np.int(ny_ / 2), i] - vort_yz_[:ny_, i],
                label='diff')
