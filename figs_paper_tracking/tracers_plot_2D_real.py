...
def main():
    ...
    # - col=2:      tracer id
    # - col=3:      CP id
    # - col=4,5:    tracer position
    # - col=6,7:    rounded tracer position
    # - col=8:      tracer radius
    ...
    lifetime_, n_lines_ = get_cp_lifetime(cp_id, n_tracers, path_tracer_file)
    print('CP lifetime computed: ', len(lifetime_), lifetime_)
    i_ini = 1
    tau = 20
    lifetime = lifetime_[i_ini:i_ini+tau:1]
    n_lines = n_lines_[i_ini:i_ini+tau:1]
    tau = len(lifetime)
    t_ini = lifetime[0]
    t_end = lifetime[-1]
    del lifetime_, n_lines_
    print('CP lifetime: ' + str(tau), lifetime)
    print('t_ini, t_end:', t_ini, t_end)
    print('line #: ', n_lines)
    print('')

    xc, yc = get_cp_center(cp_id, t_ini, tau, n_lines, lifetime, path_tracer_file)
    ...
    ''' ----- tracer coordinates ----- '''
    coordinates = get_tracer_coords(cp_id, n_lines, n_tracers, lifetime, path_tracer_file)
    print('Coordinates: ' + str(coordinates.shape))
    print(np.amax(coordinates), np.mean(coordinates))
    # var_list = ['s', 'w']
    # shift = 0
    # fig_name = 's_w'
    ...
    tracer_dist = get_tracer_distance(cp_id, n_lines, n_tracers, lifetime, path_tracer_file)
    print('Tracer Distances: ', tracer_dist.shape, tau)
    # print(tracer_dist)
    ...
    # print(np.amax(rad_))
    tracer_dist_av = np.average(tracer_dist, axis=1)
    print('Average Tracer Distances: ', tracer_dist_av.shape)
    print(tracer_dist_av)
    ...
    v_hor_int = compute_v_hor_int(u_in, v_in, ic, jc, irange, jrange, nx_, ny_,
                                  t_ini, t_end, tau, lifetime, path_out_figs)
    ...
    #
    for it, t0 in enumerate(lifetime):
        print('compute radial velocity: ', it, t0)
        ...
        # ax0.plot([tracer_dist_av[it]*dx, tracer_dist_av[it]*dx], [np.amin(v_rad_av[it,:]), np.amax(v_rad_av[it,:])], 'k', linewidth=1)
        ...
        # ax1.plot([tracer_dist_av[it]*dx, tracer_dist_av[it]*dx], [np.amin(v_rad_av[it,:]), np.amax(v_rad_av[it,:])], 'k', linewidth=1)
        # ax1.set_ylabel(r'v$_{tan}$ / ms$^{-1}$')
        # ax2.set_title('tracer distance  / m')
        # ax2.plot(lifetime, tracer_dist_av*dx, 'o-')
        # ax2.set_xlabel('time / [-]')
        # plt.tight_layout()
        # fig.savefig(os.path.join(path_out_figs, fig_name))
        # plt.close(fig)
    ...
    v_spread1 = dx*np.asarray([(tracer_dist_av[i+1]-tracer_dist_av[i])/dt_fields for i in range(len(tracer_dist_av)-1)])
    # based on position of peak in v_r
    r_vmax = np.argmax(v_rad_av[:,:rmax], axis=1)*dx
    print(r_vmax.shape, tau)
    v_spread2 = np.asarray([(r_vmax[i+1] - r_vmax[i])/dt_fields for i in range(len(tracer_dist_av)-1)])
    print('CP velocity')
    # print(tracer_dist_av)
    ...
    # fig, ax0 = plt.subplots(1, 1, figsize=(8, 4.2), sharey='none')
    fig, ax0 = plt.subplots(1, 1, figsize=(8, 6), sharey='none')
    min = np.amin(v_rad_av)
    max = np.amax(v_rad_av)+0.2
    for it, t0 in enumerate(lifetime):
        print('plotting ', it, t0, tau)
        count_color = np.double(it) / (len(lifetime)-1)
        ax0.plot(np.arange(rmax)*dx, v_rad_av[it, :rmax], '-', color=colmap(count_color), label='t=' + str((t0-t_ini)*dt_fields) + 's')
        # ax0.plot([tracer_dist_av[it] * dx, tracer_dist_av[it] * dx], [min, max], 'k', linewidth=1)
        ax0.plot([tracer_dist_av[it] * dx, tracer_dist_av[it] * dx], [min, max], '-',
                 linewidth=1, color=colmap(count_color))
        ax0.plot(tracer_dist_av[it]*dx, v_rad_av[it, tracer_dist_av[it]], 'ko', markersize=6)
        if it<(tau-1):
            i = 0
            ...
            ax0.plot([a*dx,b*dx], [v_spread1[it], v_spread1[it]], '-', linewidth=1, color=colmap(count_color))
        ...
        if it == tau-1:
            ax0.plot(tracer_dist_av[it]*dx, v_rad_av[it, tracer_dist_av[it]], 'ko', markersize=6, label='tracer')
        else:
            ax0.plot(tracer_dist_av[it]*dx, v_rad_av[it, tracer_dist_av[it]], 'ko', markersize=6)
    ...
    # ax0.add_patch(rect)
    ax0.legend(loc='center left', bbox_to_anchor=(.77, 0.7), frameon=True)
    textprops = dict(facecolor='white', alpha=0.9, linewidth=0.)
    ax0.text(2.3e2, 2., 'c)', fontsize=18, bbox=textprops)
    ax0.set_xlim(0, 5.5e3)
    # ax0.set_ylim(-0.1, max)
    fig.subplots_adjust(top=0.95, bottom=0.13, left=0.11, right=0.95, hspace=0.2, wspace=0.25)
    fig.savefig(os.path.join(path_out_figs, fig_name))
    plt.close(fig)
    #
    #
    ...
    # #     else:
    # #         ax0.plot(tracer_dist_av[it]*dx, v_rad_av[it, tracer_dist_av[it]], 'ko', markersize=6)
    # # ax1.plot(lifetime, tracer_dist_av * dx, 'o-')
    # #
    # # ax0.set_xlabel('radius r / km')
    # # ax0.set_ylabel('radial velocity / ms' + r'$^{-1}$')
    # # ax1.set_xlabel('time / [-]')
    ...
    #     ax0.plot([tracer_dist_av[it] * dx, tracer_dist_av[it] * dx], [min_av, max_av],
    #              linewidth=1, color=colmap(count_color))
    #     if it == tau-1:
    #         ax0.plot(tracer_dist_av[it]*dx, v_rad_av[it, tracer_dist_av[it]], 'ko', markersize=6, label='tracer')
    #     else:
...
def get_tracer_distance(cp_ID, n_lines, n_tracers, times, fullpath_in):
    f = open(fullpath_in, 'r')
    lines = f.readlines()

    nt = len(times)
    rad = np.zeros((nt, n_tracers))
    ...
    for it,t0 in enumerate(times):
        ...
        # print('it, t0', it, t0, timestep, count, age, age_ref, ID, cp_ID)
        i = 0
        while (timestep == t0 and ID == cp_ID):
            if i>=n_tracers:
                break
            columns = lines[count].split()
            rad[it,i] = float(columns[8])
            i += 1
            count += 1
            timestep = int(lines[count].split()[0])
            age = int(lines[count].split()[1])
...
def get_tracer_coords(cp_ID, n_lines, n_tracers, times, fullpath_in):
    ...
    #         count_start += 1
    #         ID = int(lines[count_start].split()[3])
    #     # print('t0 - cp_ID:', t0, timestep, ID, count_start)
    #     while (timestep < t0):
    #         count_start += 1
    #         timestep = int(lines[count_start].split()[0])
    #     age = int(lines[count_start].split()[1])
    #     ID = int(lines[count_start].split()[3])
    #     # print('t_end - timestep: ', t_ini, timestep, ID, count_start)
    #     print('count starts at ', count_start)
    #     count = count_start
    #     i = 0
    #     while (timestep == t0 and age == it and ID == cp_ID):
    #         columns = lines[count].split()
    ...
    for it,t0 in enumerate(times):
        ...
        age_ref = t0 - 47
        i = 0
        while (timestep == t0 and age == age_ref and ID == cp_ID):
            columns = lines[count].split()
            coords[it,i,0] = float(columns[4])
            coords[it,i,1] = float(columns[5])
            i += 1
...
def get_cp_center(cp_ID, t_ini, tau, n_lines, times, fullpath_in):
    xc = np.zeros(tau)
    yc = np.zeros(tau)
    ...
    for it,t0 in enumerate(times):
        count = n_lines[it]
        # print('cp center: it', it, t0, tau, count)
        ID = int(lines[count].split()[3])
        t_ = int(lines[count].split()[0])
        # print(ID, cp_ID, t_, t0)
        if ID == cp_ID and t_ == t0:
...
def compute_v_hor_int(u_in, v_in, xc, yc, irange, jrange, nx_, ny_,
                      t_ini, t_end, tau, times, path_out_figs):
    print('')
    print('compute v hor')
    ...
    # print('imin: ', imin, xc[0], imax, irange)
    # print('jmin: ', jmin, yc[0], jmax, jrange)
    ...
    #     ax.plot(xc_, yc_, 'xk', markersize=20)
    #     ax.plot([xc_+irange, xc_+irange], [0, ny], 'k')
    #     ax.plot([xc_-irange, xc_-irange], [0, ny], 'k')
    #     ax = axis[0, 1]
    #     ax.set_title('v in')
    ...
    #         ax.set_ylim(-5, ny_+5)
    #     for ax in axis[2,:].flat:
    #         ax.set_xlim(irange-(xc_-625), irange+(665-xc_))
    #         ax.set_ylim(jrange-(yc_-570), jrange+(610-yc_))
...
def compute_radial_vel(uv, th_field,
                       coordinates, n_tracers,
                       irange, jrange, nx, ny, ic, jc, it, t0, path_out_figs):
    x_shift_coords = -2
    ...
    # ''' plot contourf of u, v, u_rad, u_tan with velocity fields atop and tracers '''
    ...
    # for ax in axis.flat:
    #     for i in range(n_tracers):
    #         ax.plot(coordinates[it, i, 0]-ic+irange+x_shift_coords,
    #                  coordinates[it, i, 1]-jc+irange+x_shift_coords, 'ok', markersize=2)
    #
    # for ax in axis.flat:
    #     ax.plot(irange, jrange, 'xk', markersize=20)
    #     ax.set_aspect('equal')
