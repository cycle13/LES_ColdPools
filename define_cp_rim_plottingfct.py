...
def plot_outlines(perc, w_mask, rim_int, rim_out, rim_list, rim_aux, rmax2, icshift, jcshift, imin, imax, jmin, jmax,
    ...
    circle1 = plt.Circle((icshift, jcshift), np.sqrt(rmax2), fill=False, color='r', linewidth=2)
    ...
    ax = plt.subplot(ny_plots,nx_plots,4)
    plt.title('rim int')
    plt.plot([0,nx_-1],[jcshift, jcshift],'b')
    circle1 = plt.Circle((icshift, jcshift), np.sqrt(rmax2), fill=False, color='r', linewidth=1)
    ax.add_artist(circle1)
    ...
    ax = plt.subplot(ny_plots,nx_plots,9)
    plt.title('rim out')
    plt.plot([0, nx_ - 1], [jcshift, jcshift], 'b')
    circle1 = plt.Circle((icshift, jcshift), np.sqrt(rmax2), fill=False, color='r', linewidth=1, alpha=0.5)
    ax.add_artist(circle1)
...
def plot_w_field(w_c, perc, w, w_roll, w_, w_mask,
    ...
    ax1.plot([ic, ic], [0, ny - 1], 'k')
    ax1.plot([ic + gw, ic + gw], [0, ny - 1], 'k--')
    ax1.plot([0, nx - 1], [jc, jc], 'g')
    ax1.plot([0, nx - 1], [jc + gw, jc + gw], 'g--')
    ax2.imshow(w_roll.T, cmap=cm_bwr, origin='lower', vmin=-max, vmax=max)
    ax2.plot([ic + ishift, ic + ishift], [0, ny - 1], 'k', label='ic+ishift')
    ax2.plot([ic - id + ishift, ic - id + ishift], [0, ny - 1], 'b--', label='ic-id+ishift')
    ax2.plot([ic + id + ishift, ic + id + ishift], [0, ny - 1], 'b-.', label='ic+id+ishift')
    ax2.plot([0, nx - 1], [jc + jshift, jc + jshift],'g')
    ax2.plot([0, nx - 1], [jc + jshift - jd, jc + jshift - jd], 'k--')
    ax2.plot([0, nx - 1], [jc + jshift + jd, jc + jshift + jd], 'k--')
    ax2.legend(fontsize=10)
    ax3.imshow(w_.T, cmap=cm_bwr, origin='lower', vmin=-max, vmax=max)
    ax3.plot([icshift, icshift], [0, ny_ - 1], 'k')
    ax3.plot([0, nx_ - 1], [jcshift, jcshift],'g')
...
def plot_yz_crosssection(w,ic,path_out,t0):
    ...
    plt.colorbar(cax, ticks=np.arange(-np.floor(max), np.floor(max) + 1), shrink=0.5)
    plt.title('w (yz-plane), t='+str(t0)+'s')
    plt.xlabel('y')
    plt.ylabel('z')
