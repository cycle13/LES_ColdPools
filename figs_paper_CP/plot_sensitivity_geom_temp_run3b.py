...
def main():
    ...
    if dx == 100:
        ...
        path_root = '/nbi/ac/conv4/rawdata/ColdPools_PyCLES/3D_sfc_fluxes_off/single_3D_noise/run3b_dx50m/'
        # path_out_figs = '/nbi/ac/cond1/meyerbe/paper_CP_single/figs_run3_dx50m/'
        path_out_figs = '/nbi/home/meyerbe/paper_CP/figs_run3b_dx50m/'
    ...
    dTh_range_A = [2, 2, dTh_ref, 4]
    zstar_range_A = [900, 1400, zstar_ref, 1100] #[900, 900]
    rstar_range_A = [1400, 900, rstar_ref, 800] #[1300, 900]
    # dTh_range_A = [2, 2, 4]
    # zstar_range_A = [900, 1400, 1100]  # [900, 900]
    # rstar_range_A = [1400, 900, 800]  # [1300, 900]

    dTh_range_B = [3, 3, 3, 3, 3, 3]
    zstar_range_B = [500, 700, 1000, 1400, 1700, 2000]
    rstar_range_B = [1900, 1400, 1000, 700, 600, 500]
    # dTh_range_B = [3, 3, 3, 3, 3]
    # zstar_range_B = [500, 700, 1000, 1400, 1700, 2000]
    # rstar_range_B = [1900, 1400, 1000, 700, 600, 500]
    # dTh_range_B = [3, 3, 3, 3, 3]
    # zstar_range_B = [500, 700,  1000, 1700, 2000]
    # rstar_range_B = [1900, 1400, 1000, 600, 500]
    ...
    # plot_sensitivity_plots_all_v2(dTh_range_A, rstar_range_A, zstar_range_A,
    #                            dTh_range_B, rstar_range_B, zstar_range_B,
    #                            dTh_ref, rstar_ref, zstar_ref,
    #                            lvl, lvl_w, dx,
    #                            filename_stats, filename_vort,
    ...
    plot_sensitivity_plots_all_v3(dTh_range_A, rstar_range_A, zstar_range_A,
                                  dTh_range_B, rstar_range_B, zstar_range_B,
                                  dTh_ref, rstar_ref, zstar_ref,
                                  lvl, lvl_w, dx,
                                  filename_stats,
                                  path_root, path_out_figs)
...
def plot_sensitivity_plots_all(dTh_range_A, rstar_range_A, zstar_range_A,
    ...
    for i, dTh in enumerate(dTh_range_B):
        ...
        ax4.plot(time_stats, 300 * np.ones(shape=time_stats.shape), 'k-', linewidth=.5)
...
def plot_sensitivity_plots_all_v2(dTh_range_A, rstar_range_A, zstar_range_A,
                               dTh_range_B, rstar_range_B, zstar_range_B,
                                dTh_ref, rstar_ref, zstar_ref,
                               lvl, lvl_w, dx,
                               filename_stats, filename_vort,
    ...
    max_range = np.zeros(ncols, dtype=np.double)
    min_range = 9999.9*np.ones(ncols, dtype=np.double)
    fig_name = 'sensitivity_plots_all_dx'+str(dx)+'m_v2.png'
    fig, axis = plt.subplots(nrows, ncols, figsize=(ncols*4, nrows*5), sharex='col')
    ...
    ax_width = .12 #.075
    ax_height = .22#.25
    # spec = [ax_x, 0.70, ax_width, ax_height]
    spec = [ax_x, 0.73, ax_width, ax_height]
    ax00 = plt.axes(spec)
    # spec = [ax_x, 0.245, ax_width, ax_height]
    spec = [ax_x, 0.275, ax_width, ax_height]
    ax01 = plt.axes(spec)
    for i, dTh in enumerate([2,3,4]):
        zstar = [900, 1000, 900][i]
        rstar = [1300, 1000, 900][i]
    ...
    # ax0.set_title('initial configuration', fontsize=18)
    ax1.set_title('average Radius', fontsize=18)
    ax2.set_title('CP height', fontsize=18)
    # # ax3.set_title('CP volume', fontsize=18)
    ax3.set_title('potential temperature', fontsize=18)
    ax4.set_title('vertical velocity', fontsize=18)
    # # ax5.set_title('max. vorticity', fontsize=18)
    for i, dTh in enumerate(dTh_range_A):
        rstar = rstar_range_A[i]
        zstar = zstar_range_A[i]
        rootname = 'dTh'+str(dTh) + '_z'+str(zstar) + '_r'+str(rstar)
        ...
        max_range[2] = np.maximum(max_range[2], np.amax(CP_height_max))
        # max_range[3] = np.maximum(max_range[3], 1e-9 * np.amax(CP_vol))
        max_range[3] = np.maximum(max_range[3], np.amax(theta_min_k0))
        max_range[4] = np.maximum(max_range[4], np.amax(w_max))
        # max_range[5] = np.maximum(max_range[5], np.amax(vort_phi_max))
        min_range[1] = np.minimum(min_range[1], np.amin(r_av))
        min_range[3] = np.minimum(min_range[3], np.amin(theta_min))
        ax1.plot(times, r_av, '-', color=colorlist2[i], label=lbl)
        ax2.plot(time_geom, CP_height_max, '-', color=colorlist2[i], label=lbl)
        # ax3.plot(time_geom, 1e-9*CP_vol, '-', color=colorlist2[i], label=lbl)
        ax3.plot(time_stats, theta_min, '-', color=colorlist2[i], label=lbl)
        ax3.plot(time_stats,300*np.ones(shape=time_stats.shape), 'k-', linewidth=.5)
        ax4.plot(time_stats, w_max, '-', color=colorlist2[i], label=lbl)
        # ax5.plot(time_vort, vort_phi_max, '-', color=colorlist2[i], label=lbl)
    ...
    for i, dTh in enumerate(dTh_range_B):
        ...
        ax2.plot(time_geom, CP_height_max, '-', color=colorlist4_blue[i], label=lbl)
        # ax3.plot(time_geom, 1e-9*CP_vol, '-', color=colorlist4_blue[i], label=lbl)
        ax3.plot(time_stats, theta_min, '-', color=colorlist4_blue[i], label=lbl)
        ax3.plot(time_stats, 300*np.ones(shape=time_stats.shape), 'k-', linewidth=.5)
        ax4.plot(time_stats, w_max, '-', color=colorlist4_blue[i], label=lbl)
        # ax5.plot(time_vort, vort_phi_max, '-', color=colorlist4_blue[i], label=lbl)
    ...
    for i in range(2):
        ...
        axis[i,2].plot(time_geom, CP_height_max, '-', color='k', label=lbl)
        # axis[i,3].plot(time_geom, 1e-9*CP_vol, '-', color='k', label=lbl)
        axis[i,3].plot(time_stats, theta_min, '-', color='k', label=lbl)
        axis[i,4].plot(time_stats, w_max, '-', color='k', label=lbl)
        # axis[i,5].plot(time_vort, vort_phi_max, '-', color='k', label=lbl)
    ...
    axis[0,3].text(0.05, 0.86, 'environmental temperature', style='italic', transform=axis[0,3].transAxes, fontsize=12, verticalalignment='top', bbox=textprops)
    ...
    axis[1,3].text(0.05, 0.86, 'environmental temperature', transform=axis[1,3].transAxes, fontsize=12, verticalalignment='top', bbox=textprops)
    ...
    for i in range(2):
        ...
        axis[i,3].set_ylabel(r'min($\theta$)    [K]')
...
def plot_sensitivity_plots_all_v3(dTh_range_A, rstar_range_A, zstar_range_A,
                               dTh_range_B, rstar_range_B, zstar_range_B,
                                dTh_ref, rstar_ref, zstar_ref,
                               lvl, lvl_w, dx,
                               filename_stats, path_root, path_out_figs):
    ...
    max_range = np.zeros(ncols, dtype=np.double)
    min_range = 9999.9*np.ones(ncols, dtype=np.double)
    fig_name = 'sensitivity_plots_all_dx'+str(dx)+'m_v3.png'
    fig, axis = plt.subplots(nrows, ncols, figsize=(ncols*4, nrows*4.5), sharex='col')


    ''' envelopes configuration '''
    colorlist3_ = np.insert(colorlist3, 2, 'k')
    colorlist4_ = np.insert(colorlist4_blue, 3, 'k')
    colorlist5_ = np.insert(colorlist5, 2, 'k')
    zstar_max = np.maximum(np.amax(zstar_range_A), np.amax(zstar_range_B))
    rstar_max = np.maximum(np.amax(rstar_range_A), np.amax(rstar_range_B))
    irstar_max = rstar_max/dx
    nx = 1.2*irstar_max
    ...
    for i, dTh in enumerate(dTh_range_A):
        ...
        imin = nx-rstar*dxi
        imax = nx+rstar*dxi+1
        ax00.plot(x_half[imin:imax], z_max[imin:imax], color=colorlist3_[i], linewidth=2, label=lbl)
    ...
    for i, dTh in enumerate(dTh_range_B):
        ...
        imin = nx - rstar * dxi
        imax = nx + rstar * dxi + 1
        ax01.plot(x_half[imin:imax], z_max[imin:imax], '-', color=colorlist5_[i], linewidth=2, label=lbl)

    x_leg = -.4
    y_leg = -.27#-.25#-.22
    ax00.legend(loc='upper left', bbox_to_anchor=(x_leg, y_leg),
                fancybox=False, shadow=False, ncol=1, fontsize=15, frameon=False)
    ax01.legend(loc='upper left', bbox_to_anchor=(x_leg, y_leg),
                fancybox=False, shadow=False, ncol=1, fontsize=15, frameon=False)
    # for ax in [ax00, ax01]:
    #     legend = ax.legend()
    ...
    ax3 = axis[0,3]
    # ax0.set_title('initial configuration', fontsize=18)
    ax1.set_title('average Radius', fontsize=24)
    ax2.set_title('potential temperature', fontsize=24)
    ax3.set_title('vertical velocity', fontsize=24)
    # ax5.set_title('max. vorticity', fontsize=18)
    for i, dTh in enumerate(dTh_range_A):
        rstar = rstar_range_A[i]
        zstar = zstar_range_A[i]
        ...
        stats_root = nc.Dataset(os.path.join(path_in, 'data_analysis', filename_stats))
        time_stats = stats_root.groups['timeseries'].variables['time'][:]
        tmin = np.append(tmin, time_stats[0])
        tmax = np.append(tmax, time_stats[-1])
        print('---- time stats: ', time_stats[0], time_stats[-1])
        w_ = stats_root.groups['stats'].variables['w'][:,:-1,:]
        w_max = np.amax(np.amax(w_, axis=-1), axis=-1)
        del w_
        s_ = stats_root.groups['stats'].variables['s'][:,:-1,:]
        ...
        ''' tracer statistics '''
        k0 = 0
        times = np.arange(time_stats[0], time_stats[-1], 100)
        nt = len(times)
        fullpath_in = os.path.join(path_root, rootname, 'tracer_k' + str(k0), 'output')
        n_tracers = get_number_tracers(fullpath_in)
        n_cps = get_number_cps(fullpath_in)
        ...
        ax1.plot(times, r_av, '-', color=colorlist3_[i], label=lbl)
        ax2.plot(time_stats, theta_min, '-', color=colorlist3_[i], label=lbl)
        ax2.plot(time_stats,300*np.ones(shape=time_stats.shape), 'k-', linewidth=.5)
        ax3.plot(time_stats, w_max, '-', color=colorlist3_[i], label=lbl)
    ...
    for i, dTh in enumerate(dTh_range_B):
        ...
        min_range[2] = np.minimum(min_range[2], np.amin(theta_min))
        min_range[3] = 0.
        ax1.plot(times, r_av, '-', color=colorlist5_[i], label=lbl)
        ax2.plot(time_stats, theta_min, '-', color=colorlist5_[i], label=lbl)
        ax2.plot(time_stats, 300*np.ones(shape=time_stats.shape), 'k-', linewidth=.5)
        ax3.plot(time_stats, w_max, '-', color=colorlist5_[i], label=lbl)
    ...
    # filename_CPheight = 'CP_height_'+rootname+'_sth0.5.nc'
    # filename_CPvol = 'CP_volume_'+rootname+'.nc'
    # # path_ref = os.path.join(path_root, rootname)
    # path_ref = '/nbi/ac/cond1/meyerbe/ColdPools/3D_sfc_fluxes_off/single_3D_noise/run3_dx50m/dTh3_z1000_r1000_nz300'
    # print('')
    # print(rootname)
    #
    # ''' azimuthally averaged statistics '''
    # print(os.path.join(path_ref, 'data_analysis', filename_stats))
    # stats_root = nc.Dataset(os.path.join(path_ref, 'data_analysis', filename_stats))
    # time_stats = stats_root.groups['timeseries'].variables['time'][:]
    # tmin = np.append(tmin, time_stats[0])
    # tmax = np.append(tmax, time_stats[-1])
    # w_ = stats_root.groups['stats'].variables['w'][:,:-1,:]
    ...
    # for i in range(2):
    #     axis[i,1].plot(times, r_av, '-', color='k', label=lbl)
    #     axis[i,2].plot(time_stats, theta_min, '-', color='k', label=lbl)
    #     axis[i,3].plot(time_stats, w_max, '-', color='k', label=lbl)
    #
    textprops = dict(facecolor='white', alpha=0.9, linewidth=0.)
    ftsize = 21
    ax00.text(0.25, 0.97, 'a)',    transform=axis[0,0].transAxes, fontsize=ftsize, verticalalignment='top', bbox=textprops)
    axis[0,1].text(0.04, 0.97, 'b)',     transform=axis[0,1].transAxes, fontsize=ftsize, verticalalignment='top', bbox=textprops)
    axis[0,2].text(0.05, 0.97, 'c)',     transform=axis[0,2].transAxes, fontsize=ftsize, verticalalignment='top', bbox=textprops)
    axis[0,2].text(0.15, 0.95, 'environmental temperature', style='italic', transform=axis[0,2].transAxes, fontsize=12, verticalalignment='top', bbox=textprops)
    axis[0,3].text(0.05, 0.97, 'd)',     transform=axis[0,3].transAxes, fontsize=ftsize, verticalalignment='top', bbox=textprops)
    ax01.text(.25, 0.97, 'e)',     transform=axis[1,0].transAxes, fontsize=ftsize, verticalalignment='top', bbox=textprops)
    axis[1,1].text(.05, 0.97, 'f)',     transform=axis[1,1].transAxes, fontsize=ftsize, verticalalignment='top', bbox=textprops)
    axis[1,2].text(.05, 0.97, 'g)',     transform=axis[1,2].transAxes, fontsize=ftsize, verticalalignment='top', bbox=textprops)
    axis[1,2].text(0.05, 0.85, 'environmental temperature', transform=axis[1,2].transAxes, fontsize=12, verticalalignment='top', bbox=textprops)
    axis[1,3].text(.05, 0.97, 'h)',     transform=axis[1,3].transAxes, fontsize=ftsize, verticalalignment='top', bbox=textprops)

    print('')
    for i in range(2):
        axis[i,1].set_ylabel('average Radius    [km]')
        axis[i,2].set_ylabel(r'min($\theta$)    [K]')
        axis[i,3].set_ylabel('max(w)    [m/s]')
    tmin_ = np.amin(tmin)
    tmax_ = np.amax(tmax)
    for ax in axis[:,1].flat:
        y_ticks = ax.get_yticks()
    ...
    for ax in axis[:,1:].flat:
        ax.set_xlim(0,tmax_)
        ax.set_xticks(np.arange(tmin_, tmax_, step=900.))
        x_ticks = [ti/3600 for ti in ax.get_xticks()]
        ax.set_xticklabels(x_ticks)
        for label in ax.xaxis.get_ticklabels()[1::2]:
            label.set_visible(False)
...
def get_number_tracers(fullpath_in):
    ...
    print('get number tracers: ', fullpath_in + '/coldpool_tracer_out.txt')
    f = open(fullpath_in + '/coldpool_tracer_out.txt', 'r')
    lines = f.readlines()
    count = 0
