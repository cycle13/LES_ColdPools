...
def main():
    ...
    if rstar == 1100:
        ...
        t_3CP = [1500, 2200, 3600]
        t_final = [2100, 3100, 4200]
    elif rstar == 2000:
        d_range = [10, 15, 20]
        t_ini = [400, 400, 400]
        t_2CP = [600, 800, 2200]
    ...
    if rstar == 1100:
        lim_single = [0,0,0]
        lim_double = [[75, 150], [100, 150], [50, 100]]
        lim_triple = [120, 220, 250]
    elif rstar == 2000:
        lim_single = [0, 0, 0]
    ...
    #    print(path_double, id_list_d, id_list_d[d])
...
def plot_CPs_at_times(rstar, xs, ys, delta_s, delta_d, delta_t, lim_single, lim_double, lim_triple,
    ...
    for d, dstar in enumerate(d_range):
        ...
        d2 = (dstar*1.e3/2)**2
        if rad2 >= d2:
            delta_d[1] = np.floor(4.e3/dx[0]) + 2./dx[0]*np.sqrt(rad_2CP_ini[d]**2 - (dstar*1.e3)**2/4)
            [xd, yd] = [ic,jc] - delta_d*.5
        ...
        delta_d[1] = np.floor(4.e3/dx[0]) + 2./dx[0]*np.sqrt(rad_3CP_ini[d]**2 - (dstar*1.e3/2)**2)
        [xd, yd] = [ic,jc] - delta_d*.5
        rect_double2 = mpatches.Rectangle((yd, xd), delta_d[1], delta_d[0], linewidth=2, edgecolor='k', facecolor='none')
        delta_d[1] = np.floor(4.e3/dx[0]) + 2./dx[0] * np.sqrt(rad_3CP_end[d]**2 - (dstar*1.e3/2)**2)
        [xd, yd] = [ic, jc] - delta_d * .5
