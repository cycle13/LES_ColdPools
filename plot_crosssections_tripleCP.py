...
def main():
    ...
    nt = len(times)
    icoll, jcoll, ic_arr, jc_arr, x_half, y_half, z_half = define_geometry(case_name)
    global kmax
    if args.kmax:
    ...
    for var_name in var_list:
        print('-- plot '+var_name)
        root_in = nc.Dataset(os.path.join(path_in, filename_out), 'r')
        grp = root_in.groups[var_name]
        min_single = grp.variables['min_single'][:]
        min_double = grp.variables['min_double'][:]
        min_triple = grp.variables['min_triple'][:]
        max_single = grp.variables['max_single'][:]
        max_double = grp.variables['max_double'][:]
        max_triple = grp.variables['max_triple'][:]
        time_single = grp.variables['time_single'][:]
        time_double = grp.variables['time_double'][:]
        nk = grp.dimensions['nz'].size
        root_in.close()

        # profiles with min / max of var in single, double, triple window
        fig_name = 'minmax_levels_' + var_name + '_' + case_name + '.png'
        plot_minmax_profiles(var_name, max_single, max_double, max_triple,
                         min_single, min_double, min_triple, nk, fig_name)
        # plot crosssections for each level individually in time windows single/double/triple
        fig_name = 'xz_plane_' + var_name + '_single_double_triple_k' + '.png'
        plot_xz_crosssection_single_double_triple(var_name, min_single, min_double, min_triple,
                                              ...
                                              time_single, time_double, times, x_half, fig_name)
    #     # print('')
    #     # # plot crosssections at level k0 for w, s for all time-steps
    #     # for k0 in krange:
    #     #     print('---- k0: ' + str(k0) + '----')
    #     #     j0 = jcoll
    #     #     imin = (icoll - 200)
    ...
    #
    #     # ''' plot crosssections at different levels for collision time '''
    #     # krange_ = [1, 4, 10, 15]
    #     # j0 = jcoll
    #     # imin = (icoll - 200)
    #     # imax = (icoll + 200)
    #     # fig_name = 'xz_multilevel_'+var_name+'.png'
    #     # plot_xz_crosssections_multilevel('w', j0, krange_, imin, imax,
    #     #                                  icoll, jcoll, ic_arr, jc_arr,
    #     #                                  files, path_out, path_fields, fig_name)
    # # print ''
    # #
    # # # ''' ---- (c) compute mass-flux ---- '''
    ...
    # #
    # # #
    # # # ''' (d) compute horizontal convergence''''
    # #
...
def compute_minima_maxima(var_name, icoll, jcoll, ic_arr, jc_arr, times, nt, kmax):
    ...
    k_triple = np.int(np.double(200)/dx[2])
    ref_w = 0.5
    ref_s1 = entropy_from_thetas_c(300-0.1*dTh, 0)
    ref_s2 = entropy_from_thetas_c(300-0.05*dTh, 0)
    # print('delta', dTh, delta_s1, delta_s2)
    ...
    for it, t0 in enumerate(times):
        ...
        rootgrp.close()
        # variance = np.var(var[:100, :100, 0])
        if var_name == 's':
            min_single_[it, :] = np.maximum(6000, np.amin(var[icoll+10:, jcoll, :], axis=0))
            min_double_[it, :] = np.maximum(6000, np.amin(var[:icoll-3, jcoll, :], axis=0))
            min_triple_[it, :] = np.maximum(6000, np.amin(var[icoll-3:icoll+3, jcoll, :], axis=0))
        else:
        ...
        if var_name == 'w':
            if time_single == tmax and var[ic_arr[0], jcoll, 0] > ref_w:
                time_single = t0 - dt_fields
                time_double = tmax
            # print('---', var[ic_arr[0], jcoll, 0], np.amax(var[icoll-3:icoll+10, jcoll, 1]))
            # if time_double == tmax and var[ic_arr[0], jcoll, 0] < np.amax(var[icoll-3:icoll+10, jcoll, 0]):
            if time_double == tmax and var[ic_arr[0], jcoll, k_triple] < np.amax(var[icoll-3:icoll+10, jcoll, k_triple]):
                time_double = t0 - dt_fields
        elif var_name == 's':
            # ref = np.mean(var[:10, jcoll, 0])
            print('var = s: ', var[ic_arr[0], jcoll, 0], ref_s1, ref_s2)
            # if time_single == tmax and var[ic_arr[0], jcoll, 0] < (ref-3):
            if time_single == tmax and var[ic_arr[0], jcoll, 0] < (ref_s1):
                time_single = t0 - dt_fields
                time_double = tmax
            if time_double == tmax and np.amin(var[icoll, jcoll, 0]) < (ref_s2):
                time_double = t0 - dt_fields
    # if var_name == 'w':
    ...
    # plot aroudn t=t_double
    if time_double > 0 and time_double < times[-1]:
...
def plot_configuration(var, icoll, jcoll, ic_arr, jc_arr):
    ...
    ax1.set_xlim(0, nx)
    ax1.set_ylim(0, ny)
    di = np.int((d+2000)/dx[0])
    ax2.set_xlim(icoll-di,icoll+di)
    ax2.set_ylim(jcoll-di,jcoll+di)
...
# ----------------------------------
def plot_minmax_profiles(var_name, max_single, max_double, max_triple,
                         min_single, min_double, min_triple, nk, fig_name):

    print('plot minmax levels')
    print(path_out, fig_name)
    zrange = np.arange(nk) * dx[2]
    print(zrange.shape, min_single.shape, min_double.shape, min_triple.shape)
...
def plot_xz_crosssection_single_double_triple(var_name,
                                              ...
                                              max_single, max_double, max_triple,
                                              icoll, jcoll, ic_arr, jc_arr,
                                              time_single, time_double, times, x_half, fig_name_):
    ...
    imin = (icoll - 200)
    imax = (icoll + 200)
    for k0 in [0, 1, 2, 3, 4, 5, 6, 8, 10, 12, 14, 16]:
        # fig_name = 'xz_plane_' + var_name + '_single_double_triple_k' + str(k0) + '.png'
        fig_name = fig_name_[:-4] + str(k0) + '.png'
        print(fig_name)
        fig, axis = plt.subplots(3, 1, figsize=(12, 12))
        ax1 = axis[0]
        ax2 = axis[1]
        ...
        for i, ax in enumerate(axis):
            ...
            ax.plot([dx[0] * icoll, dx[0] * icoll], [min, max], 'b', linewidth=1, label='icoll')
            ax.plot([dx[0] * (icoll + 10), dx[0] * (icoll + 10)], [min, max], 'b--', linewidth=1, label='icoll+10 (imin for single)')
            ax.plot([dx[0] * (icoll - 3), dx[0] * (icoll - 3)], [min, max], 'b-.', linewidth=1, label='icoll-3 (imax for double)')
            ...
            ax.grid()
        # plot min/max
        ax1.plot([0,nx*dx[0]],[max_single[k0], max_single[k0]], 'k', linewidth=1, label='max single')
        ax2.plot([0,nx*dx[0]],[max_double[k0], max_double[k0]], 'k', linewidth=1, label='max double')
        ax3.plot([0,nx*dx[0]],[max_double[k0], max_double[k0]], 'k:', linewidth=2, label='max double')
        ax3.plot([0,nx*dx[0]],[max_triple[k0], max_triple[k0]], 'k', linewidth=1, label='max triple')
        for it, t0 in enumerate(times):
            rootgrp = nc.Dataset(os.path.join(path_fields, str(t0) + '.nc'), 'r')
            var = rootgrp.groups['fields'].variables[var_name][:, jcoll, k0]
            rootgrp.close()
            # count_color = np.double(it) / len(times)
            if t0 <= time_single:
                count_color = np.double(t0) / np.maximum(1,time_single)
                ax1.plot(x_half, var[:], color=cm(count_color), label='t=' + str(t0))
            elif t0 <= time_double:
                count_color = np.double(t0-time_single) / (time_double - time_single)
                ax2.plot(x_half, var[:], color=cm(count_color), label='t=' + str(t0))
            else:
                count_color = np.double(t0-time_double) / (times[-1] - time_double)
                ax3.plot(x_half, var[:], color=cm(count_color), label='t=' + str(t0))
